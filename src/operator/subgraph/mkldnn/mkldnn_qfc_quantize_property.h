...
#ifndef MXNET_OPERATOR_SUBGRAPH_MKLDNN_MKLDNN_QFC_QUANTIZE_PROPERTY_H
#define MXNET_OPERATOR_SUBGRAPH_MKLDNN_MKLDNN_QFC_QUANTIZE_PROPERTY_H
...
#define NAIVE_QUANTIZED_FC_NAME "_contrib_quantized_fully_connected"

class SgQFCSelector : public SubgraphSelector {
 public:
 ...
 public:
  explicit SgQFCSelector(const bool dis_all,
                         const bool dis_float_output)
      : disable_all(dis_all),
        disable_float_output(dis_float_output) {}

  bool Select(const nnvm::Node &n) override {
    if ((!disable_all) && n.op() == Op::Get(NAIVE_QUANTIZED_FC_NAME)) {
...
class SgQFCQuantizeProperty: public SubgraphProperty {
 public:
  SgQFCQuantizeProperty() {
    disable_fuse_all = dmlc::GetEnv("MXNET_DISABLE_QFC_FUSE_ALL", false);
    disable_float_output = dmlc::GetEnv("MXNET_DISABLE_QFC_FLOAT_OUTPUT", false);
  }

  static SubgraphPropertyPtr Create() {
    static const std::string &name = "QuantizedFullyConected post-quantization optimization pass";
    auto property = std::make_shared<SgQFCQuantizeProperty>();
  ...
  nnvm::NodePtr CreateSubgraphNode(const nnvm::Symbol &sym,
    ...
    DFSVisit(sym.outputs, [&](const nnvm::NodePtr &node) {
      ...
      if (node->op() == Op::Get(NAIVE_QUANTIZED_FC_NAME)) {
        fc_node = node;
  ...
  SubgraphSelectorPtr CreateSubgraphSelector() const override {
    auto selector =
        std::make_shared<SgQFCSelector>(disable_fuse_all,
...
#endif  // MXNET_OPERATOR_SUBGRAPH_MKLDNN_MKLDNN_QFC_QUANTIZE_PROPERTY_H
