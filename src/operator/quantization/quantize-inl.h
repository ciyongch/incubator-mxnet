...
static mshadow::TypeFlag GetOutputType(const QuantizeParam& param) {
  auto out_type = mshadow::kInt8;
  if (param.out_type == QuantizeParam::OutType::kAuto) {
  ...
  } else if (param.out_type == QuantizeParam::OutType::kInt8) {
    out_type = mshadow::kInt8;
  } else if (param.out_type == QuantizeParam::OutType::kUint8) {
    out_type = mshadow::kUint8;
  } else {
...
void QuantizeCompute(const nnvm::NodeAttrs& attrs,
  ...
  if (param.min_calib_range.has_value() && param.max_calib_range.has_value()) {
    ...
    if (out_type == mshadow::kUint8) {
      Kernel<quantize_unsigned, xpu>::Launch(
          s, outputs[0].Size(), outputs[0].dptr<uint8_t>(), outputs[1].dptr<float>(),
          outputs[2].dptr<float>(), inputs[0].dptr<float>(), &in_min,
          &in_max, MinValue<uint8_t>(), MaxValue<uint8_t>());
    } else if (out_type == mshadow::kInt8) {  // zero-centered quantization
      Kernel<quantize_zero_centered, xpu>::Launch(
          s, outputs[0].Size(), outputs[0].dptr<int8_t>(), outputs[1].dptr<float>(),
          outputs[2].dptr<float>(), inputs[0].dptr<float>(), &in_min,
          &in_max, MinAbs(MaxValue<int8_t>(), MinValue<int8_t>()));
    } else {
